...
public class WorldServer extends World implements IAsyncTaskHandler {
    ...
    public boolean hasPhysicsEvent = PolarisConfigImpl.IMP.USE_BLOCKPHISIC_EVENT;
        ...
        for (int chunkX = (i >> 4); chunkX <= ((l - 1) >> 4); chunkX++) {
            for (int chunkZ = (k >> 4); chunkZ <= ((j1 - 1) >> 4); chunkZ++) {
                ...
                for (TileEntity te : chunk.tileEntities.values()) {
                    if ((te.position.getX() >= i) && (te.position.getY() >= j) && (te.position.getZ() >= k) && (te.position.getX() < l) && (te.position.getY() < i1) && (te.position.getZ() < j1)) {
                        arraylist.add(te);
                    }
                }
            }
        ...
        while (!this.S[this.T].isEmpty()) {
            ...
            for (BlockActionData blockactiondata : this.S[i]) {
                if (this.a(blockactiondata)) {
                    ...
                    this.server.getPlayerList().sendPacketNearby((double) blockactiondata.a().getX(), blockactiondata.a().getY(), blockactiondata.a().getZ(), 64.0D, dimension, new PacketPlayOutBlockAction(blockactiondata.a(), blockactiondata.d(), blockactiondata.b(), blockactiondata.c()));
                }
            }
